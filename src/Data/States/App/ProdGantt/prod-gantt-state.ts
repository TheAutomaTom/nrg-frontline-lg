...
export interface ProductionSchedule {
  // Raw Data
  rawLaborKanbanGridItemsDto: LaborKanbanGridItemsDto;
}

export interface ProductionScheduleReport {
  startDate: Date;
  endDate: Date;
  totalDurationMinutes: number;
  laborItems: TicketDto[];
}
...
export const useProdGanttState = defineStore("ProdGanttState", () => {
  ...
  // Use project picker state for selected projects
  ...
  const GroupedProjectsWithSchedule = computed<GroupedProject[]>(() => {
    if (!LaborKanbanGridItems.value?.Tickets) return [];

    // Get selected project numbers
    const selectedProjectNumbers = new Set(
      projectPicker$.SelectedProjectNumbers,
    );
    ...
    LaborKanbanGridItems.value.Tickets.forEach((item) => {
